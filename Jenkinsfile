...
pipeline {
    ...
    stages {
        ...
        stage('deploy-commands') {
            steps {
                sh "docker run --rm -i --name agb-temp-deploy-cmds -e NODE_ENV=deploy -e TOKEN -e DATABASE_URL --entrypoint sh alphagamedev/alphagamebot:$AGB_VERSION -c 'node ./dist/deploy-commands.js'"
            }
        }
        stage('deploy-database') {
            steps {
                sh "docker run --rm -i --name agb-temp-migrate -e NODE_ENV=deploy -e DATABASE_URL --entrypoint sh alphagamedev/alphagamebot:$AGB_VERSION -c 'npx prisma migrate deploy'"
            }
        }
        stage('deploy') {
            steps {
